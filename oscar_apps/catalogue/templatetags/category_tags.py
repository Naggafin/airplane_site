...
category_cache = TTLCache(maxsize=1000, ttl=1200)


def category_cache_key(category):
	return "category_" + str(category.pk) + "_ancestors_and_self"


@register.simple_tag
@cached(cache=category_cache, key=category_cache_key)
def get_ancestors_and_self(category):
	"""Fetches ancestors and self for a given category, caching the result."""
	if category.is_root():
		return [category]
